...
- Stage-level `secrets` are **included** in `v1alpha1` as **opaque
  ...
  `infra-ansible` defines, validates, and resolves them — for example as
...
- Until resolution and redaction tests are green, `infra-ansible` refuses any
...
`infra-ansible/playbooks/deploy.yml` validates every declared reference and
